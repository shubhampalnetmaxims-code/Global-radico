...
const DistributorOrders: React.FC = () => {
  ...
  const [germanyOrders, setGermanyOrders] = useState<Order[]>([]);
  const [expandedOrderId, setExpandedOrderId] = useState<string | null>(null);

  useEffect(() => {
    const orders = allData.Germany.orders || [];
    setGermanyOrders(orders);
  }, [allData]);

  const handleStatusChange = (orderId: string, newStatus: string) => {
    console.log(`Order ${orderId} status changed to ${newStatus}`);
    setGermanyOrders(germanyOrders.map(o => o.id === orderId ? { ...o, status: newStatus } : o));
  };
  ...
  return (
    <div className="p-4 sm:p-8">
      <header className="flex justify-between items-center mb-8">
        <h1 className="text-2xl sm:text-3xl font-black text-slate-900 tracking-tight">Germany Customer Orders</h1>
      </header>
