          ...
          <div className="flex items-center gap-3">
            <div className="flex items-center gap-2">
              ...
              <button 
                onClick={() => {
                  const basePath = country === 'India' ? 'website-india' : 'website-germany';
                  navigate(`/${basePath}/${user ? 'profile' : 'login'}`);
                }}
              ...
              <button 
                ...
                className="p-2 text-slate-600 hover:text-slate-900 hover:bg-slate-100 rounded-full transition-all relative" 
                title={t.cart}
              >
                <svg className="h-6 w-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                {cart.length > 0 && (
                  <span className="absolute top-1 right-1 h-4 w-4 bg-indigo-600 text-white text-[8px] font-black rounded-full flex items-center justify-center shadow-lg border border-white">
                    {cart.length}
                  </span>
                )}
              </button>
